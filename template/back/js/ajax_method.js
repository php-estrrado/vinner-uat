	...
	function ajax_set_full(type,title,noty,form_id,id,offset){

		//full_form(title,noty,form_id);

		ajax_load(base_url+'index.php/'+user_type+'/'+module+'/'+type+'/'+id+'/'+offset,'list','form');

		sound('ajax_load');

	}
	function ajax_admin_vieworder(type,title,noty,form_id,id){
                $('#page-content').hide();
		ajax_load(base_url+'admin/sales/'+type+'/'+id,'demo_s','form');
		sound('ajax_load');
	}
	...
	function delete_confirm(id,msg){
        ...
        function del_confirm(id,msg,type,module,action){
		...
		bootbox.confirm(msg, function(result) {
			...
			if (result) {
				...
				ajax_load(base_url+type+'/'+module+'/'+action+'/'+id,'',action);
                                var msg = 'Deleted';
                                if(action == 'approve'){ msg = 'Approved'; }
                                else if(action == 'reject'){ msg = 'Rejected';  }
				$.activeitNoty({
			...
			if (here.attr('id') == 'mobile')
			{
				if(here.val())
				{
					...
					else 
					{
						if(here.closest('div').find('.require_alert').length)
						{
							here.closest('div').find('.require_alert').html(' * Enter valid mobile number ( must be 9 to 11 number )');
                    	} 
						else 
						{
							here.closest('div').append(''
								+'  <span id="'+take+'" class="label label-danger require_alert" >'
								+'   * Enter valid mobile number ( must be 9 to 11 number) '
								+'  </span>'
							);
						}
						can = 'no';
	...
	function form_submit(form_id,noty)
   {
	...
	function set_switchery(){
		...
		if($('#prod').length){
			...
			$(".swlat").each(function(){
				...
				new Switchery(document.getElementById('lat_'+$(this).data('id')), {color:'rgb(100, 189, 99)', secondaryColor: '#cc2424', jackSecondaryColor: '#c8ff77'});

				var changeCheckbox = document.querySelector('#lat_'+$(this).data('id'));

				changeCheckbox.onchange = function() {

				  //alert($(this).data('id'));

				  ajax_load(base_url+'index.php/'+user_type+'/'+module+'/product_latest_set/'+$(this).data('id')+'/'+changeCheckbox.checked,'prod','others');
	...
	{
		var pdf = new jsPDF(o, 'pt', 'a4'), source = $('#'+from_id)[0], specialElementHandlers = {
	...
	function export_it(type,ignore)
	{
		...
		if(type == 'pdf')
		{
			//FromaHTML('export-div',name,o,'2000');
