...
$.fn.share.settings={}})(jQuery,window);(function($){var zoome={$currentImgWrap:null,$magnifier:null,$magnifierImg:null,$magnifierState:null,init:function()
{zoome.$magnifier=$('<div id="zm-magnifier"><img style="position:relative;left:50;top:50;"/></div>').appendTo('body');zoome.$magnifierImg=$('img',zoome.$magnifier);zoome.$magnifierState=$('span',zoome.$magnifier);$(document).bind('mousemove',function(e){if(zoome.$currentImgWrap)
{zoome.movemagnifier(e);}});zoome.$magnifier.mousewheel(function(e,delta,deltaX,deltaY){var info=zoome.$currentImgWrap.get(0).zoominfo;var op=deltaY>0?1:-1;var nextZoom=Math.round((info.currentZoom+info.zoomStep*op)*10)/10.0;if(nextZoom<=info.zoomRange[1]&&nextZoom>=info.zoomRange[0])
{info.currentZoom=nextZoom;zoome.$magnifierImg.css({left:-(e.pageX-info.coords.left)*info.currentZoom+info.magnifierSize[0]/2,top:-(e.pageY-info.coords.top)*info.currentZoom+info.magnifierSize[1]/2,width:info.imgSize.width*info.currentZoom,height:info.imgSize.height*info.currentZoom});zoome.$magnifierState.text(info.currentZoom+'X');}
e.preventDefault();});},setup:function($img,options,fn)
{if($img.parent().attr('id')!='zm-magnifier')
{var i_w,i_h,largeImg,$hover_layer;$img.wrap('<div class="zm-wrap"></div>');var $imgWrap=$img.parent();if(options.hoverEf=='transparent')
{$hover_layer=$('<div class="zm-hover zm-trans"></div>').appendTo($imgWrap);}
else
{$hover_layer=$img.clone().addClass('zm-hover').appendTo($imgWrap);if(options.hoverEf=='grayscale')
...
{largeImg=$img.attr(options.largeImgAttr);new Image().src=largeImg;}
else
{largeImg=$img.attr('src');}
$imgWrap.get(0).zoominfo={defaultZoom:options.defaultZoom,zoomStep:options.zoomStep,zoomRange:options.zoomRange,showZoomState:options.showZoomState,borderSize:3,imgSize:null,coords:null,largeImg:null,currentZoom:options.defaultZoom,magnifierSize:options.magnifierSize};$imgWrap.bind('mouseover',function(){zoome.$currentImgWrap=$(this);var $img=$('img:first',this);var info=$(this).get(0).zoominfo;info.coords={left:$img.offset().left,top:$img.offset().top,right:$img.offset().left+$img.width(),bottom:$img.offset().top+$img.height()};info.largeImg=$img.attr(options.largeImgAttr)?$img.attr(options.largeImgAttr):$img.attr('src');info.imgSize={width:$img.width(),height:$img.height()};$(this).get(0).zoominfo=info;zoome.$magnifierImg.attr('src',info.largeImg).css({width:info.currentZoom*info.imgSize.width,height:info.currentZoom*info.imgSize.height});zoome.$magnifier.css({width:info.magnifierSize[0],height:info.magnifierSize[1]}).fadeIn(300);if(info.showZoomState)
{zoome.$magnifierState.text(info.currentZoom+'X').show();}
else
{zoome.$magnifierState.hide();}
$(this).find('.zm-hover').fadeIn(400);});}},movemagnifier:function(e){var info=zoome.$currentImgWrap.get(0).zoominfo;if(e.pageX>=info.coords.left&&e.pageX<=info.coords.right&&e.pageY>=info.coords.top&&e.pageY<=info.coords.bottom)
{zoome.$magnifier.css({left:e.pageX-info.magnifierSize[0]/2-info.borderSize,top:e.pageY-info.magnifierSize[1]/2-info.borderSize});zoome.$magnifierImg.css({left:-(e.pageX-info.coords.left)*info.currentZoom+info.magnifierSize[0]/2,top:-(e.pageY-info.coords.top)*info.currentZoom+info.magnifierSize[1]/2});}
else
{zoome.$currentImgWrap.find('.zm-hover').fadeOut(300);zoome.$currentImgWrap=null;zoome.$magnifier.hide();}}};(function($){var fn={img2Gray:function(img)
{try{var canvas=document.createElement('canvas');var ctx=canvas.getContext('2d');canvas.height=img.height;canvas.width=img.width;ctx.drawImage(img,0,0,img.width,img.height);var imgPixels=ctx.getImageData(0,0,img.width,img.height);for(var y=0;y<imgPixels.height;y++){for(var x=0;x<imgPixels.width;x++){var i=(y*4)*imgPixels.width+x*4;var avg=(imgPixels.data[i]+imgPixels.data[i+1]+imgPixels.data[i+2])/3;imgPixels.data[i]=avg;imgPixels.data[i+1]=avg;imgPixels.data[i+2]=avg;}}
ctx.putImageData(imgPixels,0,0,0,0,imgPixels.width,imgPixels.height);$(img).attr('src',canvas.toDataURL());}
catch(err)
{alert('Canvas can not getImageData from local or corss domain image!');}},grayscale:function($img){if($.browser.msie)
{$img.addClass("zm-gray");}
else
{fn.img2Gray($img.get(0));}},blur:function($img,radius){function BlurStack()
...
context.putImageData(imageData,0,0);$img.attr('src',canvas.toDataURL());}
catch(err)
{alert('Canvas can not getImageData from local or corss domain image!');}}};$.fn.zoome=function(options){var settings=$.extend({defaultZoom:2,largeImgAttr:'rel',zoomRange:[1,10],zoomStep:1,showZoomState:false,magnifierSize:[140,140],borderSize:3,hoverEf:'normal'},options);return this.each(function(){if(this.tagName!="IMG")
return true;if($(this).attr("src")&&$.trim($(this).attr("src"))!="")
{if(this.complete)
{zoome.setup($(this),settings,fn);}
else
{$(this).one('load',function(){zoome.setup($(this),settings,fn);});}}});};})(jQuery);(function($){var types=['DOMMouseScroll','mousewheel'];if($.event.fixHooks){for(var i=types.length;i;){$.event.fixHooks[types[--i]]=$.event.mouseHooks;}}
...
args.unshift(event,delta,deltaX,deltaY);return($.event.dispatch||$.event.handle).apply(this,args);}})(jQuery);$(function(){zoome.init();});})(jQuery)