        ...
        <div class="col-12">
            <div class="row">
                <div class="col-md-9 col-sm-8 col-12">
                    <form method="get" id="search_form" action="<?php echo base_url('home/address/search/')?>" >
                        <div class="row">
                            <div class="col-9 pr-0 mb-4">
                                <input type="text" id="adr_search" name="adr_search" class="form-control" value="<?php if(isset($adrSearch)){ echo $adrSearch; } ?>" />
                            </div>
                            <div class="col-3 pl-0 mb-4">
                                <button id="search-btn" type="submit" class="ps-btn"><i class="fa fa-search"></i> </button>
                            </div>
                        </div>
                    </form>
                </div>
                <div class="col-md-3 col-sm-4 col-12 tar">
                    <button id="addr-modal" type="button" class="ps-btn" data-toggle="modal" data-target="#addrModal"><i class="fa fa-plus"></i> Add Address</button>
                </div>
            </div>
        </div>
            ...
            if($addresses && count($addresses) >0){ 
                foreach($addresses as $adr){?>
                    <div class="col-md-6 mb-4">
                        <i id="<?php echo $adr->adrs_id?>" class="addr-edit fa fa-edit"></i>
                        ...
                        <div><?php echo $adr->name.' - '.$adr->zip?></div>
                        <p>Address Type: <i><?php echo $adr->address_label?></i></p>
                    </div><?php
                }
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('.addr-edit').on('click',function(){
            var id = this.id;
            $('#addrModal').modal('show'); $('#addr-form')[0].reset();
            $.ajax({
                url: '<?php echo base_url("home/address/customer/")?>'+id,
                method: 'GET',
                data: {id: id},
                success: function(data) { 
                var obj = JSON.parse(data); 
                    $.each( obj, function( key, value ) { 
                        $('#addrModal #'+key).val(value);
                        if(key == 'address_label'){ 
                            if(value == 'Work'){ $('#addrModal #addr_type2').prop('checked', true); }else{$('#addrModal #addr_type1').prop('checked', true); }
                        }
                        if(key == 'default_addr' && value == 1){ $('#addrModal #default_addr').prop('checked', true); }
                    });
                    $('#addrModal #default_addr').val(1);
                }
            });
        ...
        $('#search_form').on('submit', function(){
            var search = $('#adr_search').val();
            $.ajax({
                url: $('#search_form').attr('action')+$('#adr_search').val(),
                method: 'GET',
                data: {keywords: $('#adr_search').val()},
                success: function(data) { 
                    $('#address').html(data); $('#adr_search').val(search)
                }
            });
            return false;
        });
