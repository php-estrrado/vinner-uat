		...
		<table id="demo-table" class="table table-striped"  data-pagination="true" data-show-refresh="true" data-ignorecol="0,4" data-show-toggle="true" data-show-columns="false" data-search="true" >
			...
			<thead>
				<tr>
					...
					<th><?php echo translate('tax_type');?></th>
					<th><?php echo translate('Region');?></th>
					<th><?php echo translate('tax_%');?></th>
			...
			?>
                <tr>
                    ...
                    <td><?php echo $row['tax_type']; ?></td>
                   	<td><?php echo  $this->db->get_where('fed_country',array('country_id' => $row['country_id']))->row()->name; ?></td>
                   	<td><?php echo $row['tax_amount']; ?></td>
                    <td class="text-right">
                        <a class="btn btn-success btn-xs btn-labeled fa fa-wrench" data-toggle="tooltip" 
                            onclick="ajax_modal('edit','<?php echo translate('edit_tax'); ?>','<?php echo translate('successfully_edited!'); ?>','stax_edit','<?php echo $row['stax_id']; ?>')" 
                                data-original-title="Edit" 
                                    data-container="body"><?php echo translate('edit');?>
                        </a>
                        
                        <a onclick="delete_confirm('<?php echo $row['stax_id']; ?>','<?php echo translate('really_want_to_delete_this?'); ?>')" 
    ...
    <div id="coupn"></div>
	<div id='export-div'>
		<h1 style="display:none;"><?php echo translate('shipping_tax'); ?></h1>
