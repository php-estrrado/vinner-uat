<div id="content-container">
	<div id="page-title">
		<h1 class="page-header text-overflow"><?php echo translate('shipping_operators');?></h1>
	</div>
	<div class="tab-base">
		<div class="panel">
			<div class="panel-body">
				<div class="tab-content">
                            ...
                            <button class="btn btn-info btn-labeled fa fa-step-backward pull-right pro_list_btn" 
                                style="display:none;"  onclick="ajax_set_list();  proceed('edit');"><?php echo translate('back_to_operator_list');?>
                            </button>
                            <a id="addBtn" class="btn btn-primary btn-labeled fa fa-plus-circle add_pro_btn pull-right addBtn" data-toggle="tooltip" 
                                    onclick="ajax_modal('edit','<?php echo translate('create_operator')?>','<?php echo translate('operator_created!')?>','operator_detail','0')" data-original-title="<?php echo translate('create_operator');?>" data-container="body">
        ...
        $('#addBtn').on('click',function(){ 
           ...
           $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $('.modal-backdrop.fade').css('height','960px');
                    }
                });
       });
       ...
       $('body .demo-chosen-select').chosen();
        ...
        $('body').on('click','#saveOperator',function(){ 
                ...
                $.ajax({
                    ...
                    dataType: "json",
                    success: function (data) { 
                        if(data['success'] == 'success'){ $('#saveOperator').text('Saving...'); $('#operator_form').submit(); return false; }
                        else{ for (var key in data)  $("#error_"+key).html('<p>'+data[key]+'</p>'); }
                        $('#saveOperator').attr('disabled',false); $('#saveOperator').text('Submit');
                        return false;
                    }
                });
          //  }
