<?php if($this->session->userdata('curPage')){ $curPage = $this->session->userdata('curPage'); }else{ $curPage = 0; } ?>
...
<script type="text/javascript">
    $(document).ready(function(){
        var curPage = '<?php echo $this->session->userdata('curPage')?>';
        if(curPage > 0){ setTimeout(explode, 2000); }
        function explode(){ 
        //    var curPage     =   (parseInt('<?php echo $this->session->userdata('curPage')?>')+1);
       //     $(".pagination li#"+curPage).addClass('active');
        }
    }); 
</script>
<input type="hidden" name="currPage" id="currPage" value="1"/>
<div class="panel-body allprdcts" id="demo_s"> 
    <table id="events-table" class="table table-striped"  data-url="<?php echo base_url(); ?>index.php/admin/product/list_data" data-side-pagination="server" data-pagination="true" data-page-list="[5, 10, 20, 50, 100, 200]"  data-show-refresh="true" data-search="true"  data-show-export="true" >
        <thead>
            <tr>
                ...
                <th data-field="publish" data-sortable="true">
                    <?php echo translate('publish');?>
                </th>

                <th data-field="latest" data-sortable="true">
                    <?php echo translate('latest');?>
                </th>
                
                <th data-field="featured" data-sortable="true">
                    <?php echo translate('featured');?>
                </th>

                <th data-field="options" data-sortable="false" data-align="right" class="bck_end_optn">
