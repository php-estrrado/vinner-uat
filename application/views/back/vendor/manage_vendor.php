<script src="https://maps.googleapis.com/maps/api/js?v=3.exp&signed_in=false"></script>

<div id="content-container">

    <div id="page-title">

        <h1 class="page-header text-overflow"><?php echo translate('manage_profile');?></h1>

    </div>

    <div class="tab-base">

        <div class="panel">

            <div class="panel-body">

                <div class="tab-content">

                    <div class="tab-pane fade active in" id="" style="border:1px solid #ebebeb; border-radius:4px;">

                        <?php

							$vendor_data = $this->db->get_where('vendor', array('vendor_id' => $this->session->userdata('vendor_id')))->result_array();

							foreach ($vendor_data as $row) {

						?>

                        <div class="panel-heading">

                            <h3 class="panel-title"><?php echo translate('manage_details');?></h3>

                        </div>

						<?php

                            echo form_open(base_url() . 'index.php/vendor/manage_vendor/update_profile/', array(
                                'class' => 'form-horizontal',
                                'method' => 'post' ));

                        ?>

                            <div class="panel-body">

                                <div class="form-group">
                                    <label class="col-sm-3 control-label" for="demo-hor-1">
                                    	<?php echo translate('name');?>
                                     </label>
                                    <div class="col-sm-6">
                                        <input type="text" name="name" value="<?php echo $row['name']; ?>" id="demo-hor-1" class="form-control " readonly>
                                    </div>
                                </div>
								<div class="form-group">
                                    <label class="col-sm-3 control-label" for="drs_id">
                                    	<?php echo translate('drs_id');?>
                                     </label>
                                    <div class="col-sm-6">
                                        <input type="text" name="drs_id" value="<?php echo $row['drs_id']; ?>" id="drs_id" class="form-control " readonly>
                                    </div>
                                </div>

                                <div class="form-group">
                                    <label class="col-sm-3 control-label" for="demo-hor-c">
                                    	<?php echo translate('company');?>
                                    </label>
                                    <div class="col-sm-6">
                                        <input type="text" name="company" value="<?php echo $row['company']; ?>" id="demo-hor-c" class="form-control " readonly>
                                    </div>
                                </div>

                                <div class="form-group">
                                    <label class="col-sm-3 control-label" for="demo-hor-dn">
                                    	<?php echo translate('display_name');?>
                                     </label>

                                	<div class="col-sm-6">
                                    	<input type="text" name="display_name" value="<?php echo $row['display_name']; ?>" id="demo-hor-dn" class="form-control required" <?php if($row['admin_disname']=='1'){ echo 'readonly'; } ?> >
                                	</div>
                                </div>

                                <div class="form-group">
                                    <label class="col-sm-3 control-label" for="demo-hor-2">
										<?php echo translate('email');?>
                                    </label>
                                    <div class="col-sm-6">
                                      <input type="email" name="email" value="<?php echo $row['email']; ?>" id="demo-hor-2" class="form-control required" readonly>
                                    </div>
                                </div>

                               <div class="form-group">
                                    <label class="col-sm-3 control-label" for="mobile">
                                        <?php echo translate('mobile_number');?>
                                     </label>
                                    <div class="col-sm-6">
                                        <input type="text" name="mobile" id="mobile" class="form-control required" value="<?php echo $row['mobile']; ?>" >
                                    </div>
                                </div>


                                <div class="form-group">
                                    <label class="col-sm-3 control-label" for="demo-hor-4">
										<?php echo translate('address_line_1');?>
                                     </label>
                                    <div class="col-sm-6">
                                        <input type="text" name="address1" value="<?php echo $row['address1']; ?>" id="demo-hor-4" class="form-control address" >
                                    </div>
                                </div>

                                <div class="form-group">
                                    <label class="col-sm-3 control-label" for="demo-hor-ad2">
										<?php echo translate('address_line_2');?>
                                        	</label>
                                    <div class="col-sm-6">
                                        <input type="text" name="address2" value="<?php echo $row['address2']; ?>" id="demo-hor-ad2" class="form-control address" >
                                    </div>
                                </div>
								
								<div class="form-group btm_border"  style="">
									<label class="col-sm-3 control-label" for="country">
										<?php echo translate('country');?>
									</label>
									<div class="col-sm-6" >
										<select id="country" name="country_code" class="form-control required ">
										<option value="">Select Country..</option>
										  <?php
											$cntry=$this->db->get_where('fed_country',array('status'=>'1'))->result_array();
											foreach($cntry as $cntry_details)
											  {
												$sel='';
												if($cntry_details[country_id]==$row['country_code'])
												{
												  $sel=" selected";
												}
												echo "<option value='".$cntry_details[country_id]."' ".$sel.">".$cntry_details[name]."</option>";
											  }
										  ?>
									  </select>
									</div>
                            	</div>

                                <section class="col-md-8" id="lnlat" style="display:none;">
                                    <label class="input">
                                        <i class="icon-append fa fa-home"></i>   
                                        <input id="langlat" type="text" placeholder="langitude - latitude" name="lat_lang" value="<?php echo $row['lat_lang']; ?>" class="form-control " readonly>
                                    </label>
                                </section>
							
							<?php /*
                                <div class="form-group">
                                    <label class="col-sm-3 control-label" for="demo-hor-4"></label>
                                    <div class="col-sm-6">
                                        <div class="" id="maps" style="height:400px;" >
                                            <div id="map-canvas" style="height:400px;"></div>
                                        </div>
                                    </div>
                                </div>
								*/ ?>
								
                                <div class="form-group">
                                    <label class="col-sm-3 control-label" for="demo-hor-4">
										<?php echo translate('about');?>
                                     </label>
                                    <div class="col-sm-6">
                                        <textarea name="details" rows="10" class="form-control"><?php echo $row['details']; ?></textarea>
                                    </div>
                                </div>
								
								<div class="form-group">
                                    <label class="col-sm-3 control-label" for="ship_method">
										<?php echo translate('shipping_method');?>
                                        	</label>
                                    <div class="col-sm-6">
                                        <input type="text" name="ship_method" value="<?php echo $row['ship_method']; ?>" id="ship_method" class="form-control " >
                                    </div>
                                </div>
								
								<div class="form-group">
                                    <label class="col-sm-3 control-label" for="ship_region">
										<?php echo translate('shipping_region');?>
                                        	</label>
                                    <div class="col-sm-6">
                                        <input type="text" name="ship_region" value="<?php echo $row['ship_region']; ?>" id="ship_region" class="form-control " >
                                    </div>
                                </div>
                            </div>

                            <div class="panel-footer text-right">
                                <span class="btn btn-info up_chng submitter"  data-ing='<?php echo translate('updating..'); ?>' data-msg='<?php echo translate('profile_updated!'); ?>'>
									<?php echo translate('update_profile');?>
                                  </span>
                            </div>
                      		 <?php
						echo form_close();
						?>

						
						<div class="panel-heading">
                            <h3 class="panel-title"><?php echo translate('bank_details');?></h3>
                        </div>
						<?php
							$bvendor_data=$this->db->get_where('vendor', array('vendor_id' =>
																			  $this->session->userdata('vendor_id')))->row();	
                        	echo form_open(base_url() . 'vendor/business_settings/set_bank/', array(
                            'class'     => 'form-horizontal',
                            'method'    => 'post',
                            'id'        => 'gen_set',
                            'enctype'   => 'multipart/form-data'
                        	));
                    		?>
								<div class="panel-body">
									<div class="form-group">
										<label class="col-sm-3 control-label" for="bank_name">
											<?php echo translate('bank_name');?></label>
										<div class="col-sm-6">
											<div class="col-sm-">
												<input id="bank_name" class='form-control required' type="text" name="bank_name" value="<?php echo trim($bvendor_data->bank_name); ?>"/>
											</div>
										</div>
									</div>
									<div class="form-group">
										<label class="col-sm-3 control-label" for="swift_code">
											<?php echo translate('SWIFT_Code');?></label>
										<div class="col-sm-6">
											<div class="col-sm-">
												<input id="swift_code" class='form-control required' type="text" name="swift_code" value="<?php echo trim($bvendor_data->swift_code); ?>"/>
											</div>
										</div>
									</div>

									<div class="form-group">
										<label class="col-sm-3 control-label" for="account_number">
											<?php echo translate('account_number');?></label>
										<div class="col-sm-6">
											<div class="col-sm-">
												<input id="account_number" class='form-control required' type="text"
												name="account_number" value="<?php echo trim($bvendor_data->account_number); ?>"/>
											</div>
										</div>
									</div>

									<div class="form-group">
										<label class="col-sm-3 control-label" for="name_account">
											<?php echo translate('account_holder_name');?></label>
										<div class="col-sm-6">
											<div class="col-sm-">
												<input id="name_account" class='form-control required' type="text"
												name="name_account" value="<?php echo trim($bvendor_data->name_account); ?>"/>
											</div>
										</div>
									</div>

									<div class="form-group">
										<label class="col-sm-3 control-label" for="bank_branch">
											<?php echo translate('bank_branch');?>
										</label>
										<div class="col-sm-6">
											<div class="col-sm-">
												<input name="bank_branch" class="form-control" id="bank_branch" value="<?php echo trim($bvendor_data->bank_branch); ?>"/>
											</div>
										</div>
									</div>
								</div>

								<div class="panel-footer text-right">
									<span class="btn btn-info submitter" data-ing='<?php echo translate('saving'); ?>' data-msg='<?php echo translate('info_updated!'); ?>' >
                                        <?php echo translate('save');?>
                                	</span>
								</div>
						
								<?php
							echo form_close();	
						?>
						
                        <div class="panel-heading">
                            <h3 class="panel-title"><?php echo translate('change_password');?></h3>
                        </div>

							<?php
                                echo form_open(base_url() . 'index.php/vendor/manage_vendor/update_password/', array(
                                    'class' => 'form-horizontal',
                                    'method' => 'post' ));
                            	?>

                                <div class="panel-body">
                                    <div class="form-group">
                                        <label class="col-sm-3 control-label" for="demo-hor-5">
                                        	<?php echo translate('current_password');?>
                                        </label>
                                        <div class="col-sm-6">
                                            <input type="password" name="password" value="" id="demo-hor-5" class="form-control required">
                                        </div>
                                    </div>

                                    <div class="form-group">
                                        <label class="col-sm-3 control-label" for="demo-hor-6">
                                        	<?php echo translate('new_password*');?>
                                        </label>
                                        <div class="col-sm-6">
                                            <input type="password" name="password1" value="" id="demo-hor-6" class="form-control pass pass1 required">
                                        </div>
                                    </div>

                                    <div class="form-group">
                                        <label class="col-sm-3 control-label" for="demo-hor-7">
                                        	<?php echo translate('confirm_password');?>
                                         </label>
                                        <div class="col-sm-6">
                                            <input type="password" name="password2" value="" id="demo-hor-7" class="form-control pass pass2 required">
											<div id="pass_note">
                                        	</div>
                                        </div>
                                    </div>

                                </div>

                                <div class="panel-footer text-right">
                                    <span class="btn btn-info pass_chng disabled" disabled='disabled' data-ing='<?php echo translate('updating..'); ?>' data-msg='<?php echo translate('password_updated!'); ?>'>
                                    	<?php echo translate('update_password');?>
                                    </span>
                                </div>
                        		<?php
							echo form_close();
                          }
                        ?>
                    </div>
                </div>
            </div>
        <!--Panel body-->
        </div>
    </div>
</div>



<script type="text/javascript">


$("#demo-hor-3").blur(function()
					  {
        var phone = $("#demo-hor-3").val();
      
        var phoneno = /^\+?([0-9]{2})\)?[-. ]?([0-9]{10})$/;
        //var phoneno = /^\?([0-9]{10})$/;
        if(phone.match(phoneno))
        {
            $("#phonenote").html('');
            $(".up_chng").removeAttr("disabled");
            $(".up_chng").removeClass("disabled");
        } 
        else 
        {
            $("#phonenote").html('' + '  <span class="require_alert" >' + '      <?php echo translate('Enter_valid_Phone_Number(+00 0000000000)'); ?>' + '  </span>');
          
           $(".up_chng").attr("disabled", "disabled");
           $(".up_chng").addClass("disabled");
        }
        
    });

	$(".pass").blur(function() 
	{
		$("#pass_note").html('');
		var pass1 = $(".pass1").val();
		var pass2 = $(".pass2").val();
		if (pass1 !== pass2) 
		{
		  if(pass2)
		  {
			$("#pass_note").html('' + '  <span class="require_alert" >' + '      <?php echo translate('password_mismatched'); ?>' + '  </span>');
		  }

			$(".pass_chng").attr("disabled", "disabled");
			$(".pass_chng").addClass("disabled");

		} 
		else if (pass1 == pass2 &&  (pass1!='' && pass2!='') ) 
		{
			$("#pass_note").html('');
			$(".pass_chng").removeAttr("disabled");
			$(".pass_chng").removeClass("disabled");
		}

	});

	

	$('.pass_chng').on('click', function() {

	

		//alert('vdv');

		var here = $(this); // alert div for show alert message

		var form = here.closest('form');

		var can = '';

		var ing = here.data('ing');

		var msg = here.data('msg');

		var prv = here.html();

	

		//var form = $(this);

		var formdata = false;

		if (window.FormData) {

			formdata = new FormData(form[0]);

		}

	

		var a = 0;

		var take = '';

		form.find(".required").each(function() {

			var txt = '*<?php echo translate('required'); ?>';

			a++;

			if (a == 1) {

				take = 'scroll';

			}

			var here = $(this);

			if (here.val() == '') {

				if (!here.is('select')) {

					here.css({

						borderColor: 'red'

					});

	

					if (here.closest('div').find('.require_alert').length) {

	

					} else {

						sound('form_submit_problem');

						here.closest('div').append('' + '  <span id="' + take + '" class="label label-danger require_alert" >' + '      ' + txt + '  </span>');

					}

				}

				var topp = 100;

	

				$('html, body').animate({

					scrollTop: $("#scroll").offset().top - topp

				}, 500);

				can = 'no';

			}

	

			take = '';

		});

	

		if (can !== 'no') {

			$.ajax({

				url: form.attr('action'), // form action url

				type: 'POST', // form submit method get/post

				dataType: 'html', // request type html/json/xml

				data: formdata ? formdata : form.serialize(), // serialize form data 

				cache: false,

				contentType: false,

				processData: false,

				beforeSend: function() {

					here.html(ing); // change submit button text

				},

				success: function(data) {

					here.fadeIn();

					here.html(prv);

					if (data == 'updated') {

						$.activeitNoty({

							type: 'success',

							icon: 'fa fa-check',

							message: msg,

							container: 'floating',

							timer: 3000

						});

                        document.getElementById("demo-hor-5").value="";
                        document.getElementById("demo-hor-6").value="";
                        document.getElementById("demo-hor-7").value="";


					} else if (data == 'pass_prb') {

						$.activeitNoty({

							type: 'danger',

							icon: 'fa fa-check',

							message: '<?php echo translate('incorrect_password!'); ?>',

							container: 'floating',

							timer: 3000

						});

					}

				},

				error: function(e) {

					console.log(e)

				}

			});

		} else {

			sound('form_submit_problem');

			return false;

		}

	});



    var base_url = '<?php echo base_url(); ?>';



</script>



<script>

    

    $(document).ready(function()
	{

        

    });


        $('.address').on('blur', function(){

			$('#langlat').val('');

            //set_cart_map();

        });


</script>


<style>
	#pass_note
	{
		color:red;
	}
</style>

