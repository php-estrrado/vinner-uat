    ...
    function category($para1 = '', $para2 = '')
    {
        ...
        if ($para1 == 'do_add') 
		{
          ...
          /* $cat_name = $this->input->post('category_name');
            ...
            $this->db->insert('category', $data); $insId = $this->db->insert_id();
            if($_FILES["image"] && $insId){
                $fileName               =   $_FILES["image"]["name"];
                $ext                    =   end((explode(".", $fileName)));
                if(move_uploaded_file($_FILES["image"]['tmp_name'], 'uploads/category_image/image_' . $insId . '.'.$ext)){
                    $this->db->where('category_id',$insId)->update('category',['image'=>'image_'. $insId . '.'.$ext]);
                }
            }
            if($insId){ $this->db->insert('sub_category',['sub_category_name'=>$this->input->post('category_name'),'category'=>$insId]); }
            recache();
        } else if ($para1 == 'edit') {
            ...
            if($_FILES["image"]){
                $fileName               =   $_FILES["image"]["name"];
                $ext                    =   end((explode(".", $fileName)));
                if(move_uploaded_file($_FILES["image"]['tmp_name'], 'uploads/category_image/image_' . $para2 . '.'.$ext)){
                    $this->db->where('category_id',$para2)->update('category',['image'=>'image_'. $para2 . '.'.$ext]);
                }
            }
            ...
            $this->db->where('category', $para2)->delete('sub_category');
            recache();
        } elseif ($para1 == 'list') {
 ...
 /*inventory managemant*/
    ...
    function coupon($para1 = '', $para2 = '', $para3 = '')
    {
        ...
        if ($para1 == 'do_add') {
            ...
            $page_data['all_coupons'] = $this->db->where(array('abandoen_coupon'=>'0'))->get('coupon')->result_array();
 ...
 function report_sale($para1 = '', $para2 = '')
    ...
    function product($para1 = '', $para2 = '', $para3 = '')
    {
        ...
        {         
            ...
            //  echo '<pre>'; print_r($this->input->post()); echo '</pre>'; die;
            $vendorPrices               =   $this->input->post('price_vendor');
            $data['title']              = $this->input->post('title');
            $data['category']           = $this->input->post('category');
            $data['description']        = $this->input->post('description');
            ...
            $data['sub_category']       = $this->input->post('category');;
            ...
            $data['brand']              = implode(',',$this->input->post('brand'));
            $data['unit']               = $this->input->post('unit');
                ...
                if($this->input->post('request_demo')){ 
                $data['request_demo']     = $this->input->post('request_demo'); }else{ $data['request_demo'] = 0; }
                ...
                $id = $this->db->insert_id();
                $this->crud_model->addPrdBrands($id,$this->input->post('brand'));
                if($vendorPrices){ foreach($vendorPrices as $vId=>$vPrice){
                        $this->db->insert('vendor_prices',['vendor_id'=>$vId,'prd_id'=>$id,'price'=>$vPrice]);
                    } }

    			$this->benchmark->mark_time();
                $this->crud_model->file_up("primary_image", "product", $id);
                if($num_of_imgs>0)
        ...
        //    $data['num_of_imgs']        = $num + $num_of_imgs;
            ...
            {
                $this->crud_model->file_up("primary_image", "product", $para2);
            }
            ...
            $this->crud_model->addPrdBrands($para2,$this->input->post('brand'));
            recache();
            ...
            if(count($rid ) > 0)
                ...
                if($vendorPrices){ foreach($vendorPrices as $vId=>$vPrice){
                    if($this->db->get_where('vendor_prices',['vendor_id'=>$vId,'prd_id'=>$para2,'status'=>1])->num_rows() > 0){
                        $this->db->where(['vendor_id'=>$vId,'prd_id'=>$para2,'status'=>1])->update('vendor_prices',['price'=>$vPrice]);
                    }else{ $this->db->insert('vendor_prices',['vendor_id'=>$vId,'prd_id'=>$para2,'price'=>$vPrice]); }
                } }
                recache();
            ...
            if(isset($currPage)){ }else{  $currPage = 1; }
            $result = array(
                            'offset' => $offset,
                            'currPage' => $currPage,
                             'total' => $total,
        ...
        recache();
         ...
         elseif ($para1 == 'sub_by_cat') {
            echo $this->crud_model->select_html('sub_category', 'sub_category', 'sub_category_name', 'add', 'demo-chosen-select', '', 'category', $para2, 'get_sub_res');
            ...
            echo $this->crud_model->select_html('product', 'product', 'title', 'add', 'demo-chosen-select required', '', 'category', $para2, 'get_pro_res','',1);
        } 
    ...
    function sales($para1 = '', $para2 = '',$para3="")
    {
        ...
        elseif ($para1 == 'send_invoice') {
            ...
            $delivery_status = $this->db->get_where('sale', array('sale_id' => $para2))->row()->delivery_status;
            $page_data['delivery_status'] = $delivery_status;
...
function quote_recepients($para1 = "")
    ...
    */
    ...
    function general_settings($para1 = "", $para2 = "")
    {
        ...
        if ($para1 == "refund_cancellation_policy") 
        {
            $this->db->where('type', "refund_cancellation_policy");
            $this->db->update('general_settings', array(
                'value' => $this->input->post('refund_cancellation_policy')
            ));
        }
        if ($para1 == "frequently_asked_questions") 
        {
            $this->db->where('type', "frequently_asked_questions");
            $this->db->update('general_settings', array(
                'value' => $this->input->post('frequently_asked_questions')
            ));
        }
        
        if ($para1 == "cancellation_replacement_policy") 
        {
            $this->db->where('type', "cancellation_replacement_policy");
            $this->db->update('general_settings', array(
                'value' => $this->input->post('cancellation_replacement_policy')
            ));
        }
    ...
    function product_group($para1 = '', $para2 = '', $para3 = '')
    {
        ...
        {   
         // echo '<pre>'; print_r($this->input->post()); echo '</pre>'; die;
            ...
            $qty                        =   $this->input->post('quantity');  
            $prdName                    =   $this->input->post('p'); 
            $data['related_products']   =   implode(',',$this->input->post('relatedproducts'));
            $choice_titles              =   $this->input->post('op_title');
            $choice_types               =   $this->input->post('op_type');
            $choice_no                  =   $this->input->post('op_no');
            if(count($choice_titles ) > 0)
...
/* Vendor Product view,  */
    function vendor_product($para1 = '', $para2 = 0, $para3 = '')
    {
       if (!$this->crud_model->admin_permission('product')) 
        {
            redirect(base_url() . 'index.php/admin');
        ...
        elseif ($para1 == 'list'){
            $products                   =   $this->crud_model->getWherehouseProducts($para2);
            $page_data['all_products']  =   $products;
            $page_data['all_vendor']    =   $this->db->get('vendor')->result_array();
            $page_data['page_name']     =   "vendor_product";
            $page_data['vendor_id']     =   $para2;
            $page_data['vendorId']      =   $para2;
	...
	function emailtemplate($para1 = '', $para2 = '0')
	{
     ...
     /* request demo edit, view, delete */
    function demo_request($para1 = '', $para2 = '', $para3 = '')
    {
        if (!$this->crud_model->admin_permission('demo')) {
            redirect(base_url() . 'index.php/admin');
        }
        if ($para1 == 'edit') {
            $page_data['demo_req_data'] = $this->db
            ->select('request_demo.*,product.request_demo')
            ->join('product','product.product_id = request_demo.product_id', 'left')
            ->get_where('request_demo', array(
                'request_demo.id' => $para2
            ))->result_array();
            $this->load->view('back/admin/demo_request_edit', $page_data);
        } 
        else if ($para1 == 'view') {
            $page_data['request_id'] = $para2;
            $page_data['demorequest_data'] = $this->db
            ->select('request_demo.*,product.request_demo')
            ->join('product','product.product_id = request_demo.product_id', 'left')
            ->get_where('request_demo', array(
                'request_demo.id' => $para2
            ))->result_array();
            $this->load->view('back/admin/demo_request_view', $page_data);
        }
        elseif ($para1 == "update") {
            $data['name'] = $this->input->post('name');
            $data['email'] =  $this->input->post('email');
            ...
            $data['time'] =  date('H:i', strtotime($this->input->post('time')));
            $data['product_id'] =  $this->input->post('product');
            $data['remarks'] =  $this->input->post('remarks');
            $this->db->where('id', $para2);
            $this->db->update('request_demo', $data);
        } elseif ($para1 == 'delete') {
            $this->db->where('id', $para2);
            $this->db->set('status', 0);
            $this->db->update('request_demo');
        } elseif ($para1 == 'list') {
            $this->db->order_by('id', 'desc');
            $page_data['all_demo_requests'] = $this->db->where('status',1)->get('request_demo')->result_array(); 
            $this->load->view('back/admin/demo_request_list', $page_data);
        } 
         elseif ($para1 == 'publish_set') {
            $product = $para2;
            if ($para3 == 'true') {
            ...
            $page_data['page_name']      = "demo_request";
            $page_data['all_demo_requests'] = $this->db->where('status',1)->get('request_demo')->result_array(); 
            $this->load->view('back/index', $page_data);
        }
    }

     /* request service edit, view, delete */
    function request_service($para1 = '', $para2 = '', $para3 = '')
    {
        if (!$this->crud_model->admin_permission('demo')) {
            redirect(base_url() . 'index.php/admin');
        }
        if ($para1 == 'edit') {
            $page_data['service_req_data'] = $this->db->get_where('request_service', array(
                'request_id' => $para2
            ))->result_array(); 
            $this->load->view('back/admin/request_service_edit', $page_data);
        } elseif ($para1 == "update") {
            $data['name'] = $this->input->post('name');
            $data['email'] =  $this->input->post('email');
            $data['mobile'] =  $this->input->post('phone');
            ...
            $this->db->where('request_id', $para2);
            $aaa = $this->db->update('request_service', $data); 
            
        } elseif ($para1 == 'delete') {
            $this->db->where('request_id', $para2);
            $this->db->set('status', 0);
            $this->db->update('request_service');
        } elseif ($para1 == 'list') {
             $this->db->order_by('request_id', 'desc');
            $page_data['all_request_service'] = $this->db->where('status',1)->get('request_service')->result_array(); 
            $this->load->view('back/admin/request_service_list', $page_data);
        } 
        else if ($para1 == 'view') {
            $page_data['request_id'] = $para2;
            $page_data['servicerequest_data'] = 
            $this->db->get_where('request_service', array(
                'request_id' => $para2
            ))->result_array();
            $this->load->view('back/admin/request_service_view', $page_data);
        }
         elseif ($para1 == 'publish_set') {
            $product = $para2;
            if ($para3 == 'true') {
            ...
            $this->db->where('request_id', $product);
            $this->db->update('request_service', $data);
        } else {
    ...
    function prdImgUpload($id=0){ 
            $imgId = $this->crud_model->file_up("uploadfile", "product", $id, 'multi');
            $product    =   $this->db->get_where('product',['product_id'=>$id])->row();
            if($product){ $this->db->where('product_id',$id)->update('product',['num_of_imgs'=>($product->num_of_imgs+1)]); }
            echo 'success_'.$imgId; die;
    }
    
    /* Abandon group edit, view, delete */
    function abandon_group($para1 = '', $para2 = '', $para3 = '')
    {
        if (!$this->crud_model->admin_permission('coupon')) {
            redirect(base_url() . 'index.php/admin');
        }
        if ($para1 == 'do_add') 
        {
            ...
            $data['till'] = $this->input->post('till');
            $data['status'] = 'ok';
            $data['abandoen_coupon'] = '1';
            $data['added_by'] = json_encode(array('type'=>'admin','id'=>$this->session->userdata('admin_id')));
            $data['spec'] = json_encode(array(
                                'set_type'=>'product',
                                'set'=>json_encode(array($this->input->post('product'))),
                                'discount_type'=>$this->input->post('discount_type'),
                                'discount_value'=>$this->input->post('discount_value'),
                                'shipping_free'=>''
                            ));
            ...
            if($group_id)
            {
                $data_grpmember=array('grp_product_id'=>$this->input->post('product'),'grp_coupon_id'=>$group_id);
                $data_grpmember['grp_cart_start_date']=$this->input->post('from_cart');
                $data_grpmember['grp_cart_end_date']=$this->input->post('to_cart');
                
                $group_customer=$this->input->post('group_customer[]');
                foreach ($group_customer as $memb)
                {
                    $data_grpmember['member_user_id']=$memb;
                    $this->db->insert('abandon_group_members', $data_grpmember);
                }
        ...
        else if ($para1 == 'edit') 
        {
            ...
            //$page_data['page_name']      = "abandon_group_edit";
            $page_data['crt_from']=$page_data['crt_to']='';$page_data['members_array']=array();    
            $grp_members_qry=$this->db->get_where('abandon_group_members', array('grp_coupon_id' => $para2 )); 
            if($grp_members_qry->num_rows()>0) 
            {
                $member_data =$grp_members_qry->row();
                $page_data['crt_from']=$member_data->grp_cart_start_date;
                $page_data['crt_to']=$member_data->grp_cart_end_date;
                $member_array =$grp_members_qry->result_array();
                $arr = array_map (function($value){
                    return $value['member_user_id'];
                } , $member_array);

                $page_data['members_array']=$arr;
            }   
            $this->load->view('back/admin/abandon_group_edit',$page_data);
            //$this->load->view('back/index', $page_data);
        } 
        elseif ($para1 == "update") 
        {
            ...
            $data['spec'] = json_encode(array(
                                'set_type'=>'product',
                                'set'=>json_encode(array($this->input->post('cupn_product'))),
                                'discount_type'=>$this->input->post('discount_type'),
                                'discount_value'=>$this->input->post('discount_value'),
                                'shipping_free'=>''
                            ));
            ...
            $data_grpmember=array('grp_product_id'=>$this->input->post('cupn_product'),'grp_coupon_id'=>$para2);
            $data_grpmember['grp_cart_start_date']=$this->input->post('grpfrom_cart');
            $data_grpmember['grp_cart_end_date']=$this->input->post('grpto_cart');
            
            $group_customer=$this->input->post('group_customer[]');//$membids = array();
            foreach ($group_customer as $memb)
                {
                    $data_grpmember['member_user_id']=$memb;
                    $inall=$this->db->where($data_grpmember)->get('abandon_group_members');
                    if($inall->num_rows()==0)
                    {
                        $this->db->insert('abandon_group_members', $data_grpmember);
                    }
                }
            $membidss = implode(",",$group_customer);
            $idss = explode(",", $membidss);
            //$this->db->insert('abandon_group_members', array('member_user_id'=>$membidss));
            $this->db->where_not_in('member_user_id', $idss)->where(array('grp_product_id'=>$this->input->post('cupn_product'),'grp_coupon_id'=>$para2))->delete('abandon_group_members');
        } 
        elseif ($para1 == 'delete') 
        {
            $this->db->where('coupon_id', $para2);
        ...
        elseif ($para1 == 'add') 
        {
            $product_names=array(''=>'Select Product');
            $this->db->select('product_id,title')->where(array('type' => 'single','status'=>'ok'));
            $productids=$this->db->order_by('title')->get('product')->result();
            foreach($productids as $prrow)
            {
                $product_names[$prrow->product_id]=$prrow->title;
            }
            $page_data['product_list']  = $product_names;
            // $page_data['page_name']     = "abandon_group_add";
            $this->load->view('back/admin/abandon_group_add',$page_data);
        } 
        elseif ($para1 == 'publish_set') 
        {
            $product = $para2;
        ...
        {
            $this->db->order_by('coupon_id', 'desc');
            $page_data['all_coupons'] = $this->db->where(array('abandoen_coupon'=>'1'))->get('coupon')->result_array();
            $this->load->view('back/admin/abandon_group_list', $page_data);
        } 
        else 
        {
            $page_data['page_name']      = "abandon_group";
            $page_data['all_coupons'] = $this->db->where(array('abandoen_coupon'=>'1'))->get('coupon')->result_array();
            $this->load->view('back/index', $page_data);
        }
    }

    /* cart customer dropdown html */
    function cart_customerdrop($para1 = '', $para2 = '', $para3 = '')
    {
        $para1 = $this->input->get('prid'); $para2 = $this->input->get('frmdate');  $para3 = $this->input->get('todat');
        $custmr='<span style="color:red">No Customers Found</span>';
        if($para1 && ($para3=='' || $para2==''))
        {
            $custmr='<span style="color:red">Please select cart date range </span>';
        }
        if($para1=='')
        {
            $custmr='<span style="color:red">Please select Product </span>';
        }

        $custmr.=form_dropdown('group_customer[]', array(),'','class="form-control demo-cs-multiselect required" id=group_customer required multiple placeholder="No Customers"');  
        if($para1 && $para2 && $para3)
        {
            $crt_custmr= $this->db->query("SELECT DISTINCT user_id FROM cart_added_items where prd_id='$para1' and (date(added_on)>='$para2' and date(added_on)<='$para3')")->result_array();
            ...
            if(count($arr)>0)
            {
                ...
                foreach($productids as $prrow)
                {
                    $customer_names[$prrow->user_id]=ucfirst($prrow->username).'('.$prrow->mobile.')';
                }
                $custmr=form_dropdown('group_customer[]', $customer_names,'','class="form-control demo-chosen-multiselect required" id=group_customer required multiple data-placeholder="Select Customer"');  
            }
        }
        echo $custmr;
    ...
    function certificates($para1='',$para2=''){    
        if($para1 == 'add'){ 
            $result                 =   $this->crud_model->file_up('file','certificates',$insId,'');
            if($result){ 
                $this->db->insert('product_certificates',['title'=>$this->input->post('title'),'file'=>$result,'created_on'=>date('Y-m-d H:s:i')]);
                $insId              =   $this->db->insert_id(); 
            }
            if($insId){ $this->session->set_flashdata('success', 'Certificate uploaded successfully!'); }
            else{ $this->session->set_flashdata('error', 'Some error occured. Please try after some time.'); }
            redirect(base_url('admin/certificates'));
        }else if($para1 == 'delete'){ 
            $certi          =   $this->db->where('id',$para2)->get('product_certificates')->row();
            unlink(FCPATH.'uploads/certificates/'.$certi->file);
            $this->db->where('id',$para2)->delete('product_certificates');
            $this->session->set_flashdata('success', 'Certificate deleted successfully!');
            redirect(base_url('admin/certificates'));
        }else{
            $data['title']          =   'Product Certificates';
            $data['page_name']      =   'product/certificates';
            $data['certificates']   =   $this->db->get_where('product_certificates',['status'=>1])->result();
