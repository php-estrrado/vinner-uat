<?php
		...
		$shipPhone      =   '';
            ...
            //    echo $weightUnit.' -- '.$packDimUnit.' -- '.$weight.' -- '.$packLength.' -- '.$packWidth.' -- '.$packHeight; die;
                if($_POST['vendor'] > 0){
...
//								$tracking_type = $this->getTrackingType(STANDARD_OVERNIGHT);
        ...
        {
            $lastRec =   $marine->addLabelDatail($data,$fedex_charge); ?>
        ...
        if($MasterTrackingId > 0 || $MasterTrackingId == '')
        {
            ...
            foreach ($data as $res){
              //  echo '<pre>'; print_r($lastRec); echo '</pre>';
                ...
                $lastRec =   $marine->addLabelDatail($res,$fedex_charge); 
